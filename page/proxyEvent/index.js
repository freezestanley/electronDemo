 ...
 * event []
 * var a = { id: 'A' }
   var c = guardCtrler([function (evt, next) {
     console.log(this)
     console.log(evt)
     ...
     next()
   }], 'this is guardCtrler', a)
   c()
 */
function guardCtrler (event = [], param, target) {
  let eventlist = event.filter(e => e)
  if (eventlist.length < 1) return function () {}
  let cur = eventlist.shift()
  return function (evt = param) {
    cur.call(target, evt, guardCtrler(eventlist, evt, target))
  }
}
              ...
              if (typeof listener === 'object') {
                (guardCtrler([
                  ...
                  function (evt, next) {
                    ...
                    guardCtrler(target['__proxy']['__afterGuard'], evt, this)()
