...
export default class clairvoyant {
  ...
  addBaseEvent() {
    ...
    window.addEventListener("scroll", debounce(ev => this.observer({type: "scroll", evt: ev}), delay), {noShadow: true});
  }
  ...
  mutationWatch() {
    ...
    let mutationEventCallback = (mutationsList, itself) => {
      ...
      // let currentNode = []; ele.map((e, idx, arr) => {   if (e.type === "childList"
      // && e.addedNodes.length > 0) {     for (let i = 0; i < e.addedNodes.length;
      // i++) {       let ne = e.addedNodes[i];       if (ne.nodeType != 1) continue
      // console.log(ne.querySelectorAll("input"))       let netagname = ne.tagName &&
      // ne.tagName.toLocaleLowerCase();       if (         netagname === "input" ||
      //     netagname === "select" ||         netagname === "textarea"       ) {
      // currentNode.push(ne);       }       let query = [
      // ...ne.querySelectorAll("input"), ...ne.querySelectorAll("textarea"),
      // ...ne.querySelectorAll("select")      ];       currentNode =
      // currentNode.concat(query);     }   } });   let currentNode = [
      // ...document.querySelectorAll("input"),
      // ...document.querySelectorAll("textarea"),
      // ...document.querySelectorAll("select")   ]   currentNode =
      // currentNode.filter(v =>  _this.formlist.indexOf(v) === -1)
      // currentNode.map((cNode, index, array) => {     if (cNode.type === "select") {
      //       cNode.addEventListener("change", _this.selectChangEvent.bind(_this), {
      //     noShadow: true       });     } else if (       cNode.type === "radio" ||
      //      cNode.type === "checkbox"     ) { cNode.addEventListener("change",
      // _this.inputChangEvent.bind(_this), { noShadow: true       });     } else {
      // cNode.addEventListener("input", _this.inputChangEvent.bind(_this), {
      // noShadow: true       })     } })   _this.formlist =
      // _this.formlist.concat(currentNode)
    }
        ...
        if (scrollNode) {
          ...
          if (!isListener) {
            ...
            scrollNode.addEventListener("mouseenter", () => {
              scrollNode.addEventListener("scroll", domScroll, {noShadow: true});
            }, {noShadow: true});

            scrollNode.addEventListener("mouseleave ", () => {
              scrollNode.removeEventListener("scroll", domScroll, {noShadow: true});
            }, {noShadow: true});
          }
        }
      }, delay), {noShadow: true});
    ...
    const target = {
      ...
      click: function () {
        ...
        if (link && link.target === "_blank") {
          ...
          eventType.SPLIT_DATA}${readXPath(evt.target)}${point}${eventType.SPLIT_LINE}`;
        } else {
      ...
      scroll: function () {
        ...
        param.r = `${param.r}${event}${eventType.SPLIT_DATA}${readXPath(target)}${eventType.SPLIT_DATA}${scroll}${eventType.SPLIT_DATA}${
      ...
      touchdrag: function () {
        ...
        param.r = `${param.r}${event}${eventType.SPLIT_DATA}${readXPath(evt.target)}${eventType.SPLIT_DATA}S:${evt.changedTouches[0].screenX}-${
        evt.changedTouches[0].screenY}${eventType.SPLIT_DATA}E:${
        evt._startPoint.changedTouches[0].screenX}-${evt._startPoint.changedTouches[0].screenY}${eventType.SPLIT_DATA}${
        eventType.SPLIT_LINE}`;
        param.m = `${param.r}${event}${eventType.SPLIT_DATA}${readXPath(movement.ele)}${eventType.SPLIT_DATA}W:${movement.rect.width}${
        eventType.SPLIT_DATA}H:${movement.rect.height}${
        eventType
          .SPLIT_DATA}EX:${movement
          .delta
          .x
          .trim()}${
        eventType
          .SPLIT_DATA}EY:${movement
          .delta
          .y
          .trim()}${
      ...
      paint: function () {
        ...
        evt.changedTouches[0].screenX}-${evt.changedTouches[0].screenY}${eventType.SPLIT_DATA}${
        eventType.SPLIT_DATA}${eventType.SPLIT_LINE}`;
        _self
          .wsSocket
