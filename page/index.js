...
export default class clairvoyant {
    ...
    let mutationEventCallback = (mutationsList, itself) => {
      ...
      currentNode.map((cNode, index, array) => {
        ...
        if (cNode.type != 'radio' || cNode.type != 'checkbox' || cNode.type != 'select') {
          cNode.addEventListener('focus', _this.inputFocusEvent.bind(_this), {noShadow: true})
          cNode.addEventListener('blur', _this.inputBlurEvent.bind(_this), {noShadow: true})
        }
      })
  ...
  observer(obj) {
    ...
    const target = {
      openpage: function () {
        // const ck = cookie.getCookie('ISEE_BIZ') const ck_cache =
        // cookie.getCookie('ISEE_BIZ_CACHE')
        param.wh = `${document.documentElement.clientWidth}x${document.documentElement.clientHeight}`
        // if (!ck_cache || ck_cache !== ck) {   cookie.setCookie('ISEE_BIZ_CACHE', ck)
