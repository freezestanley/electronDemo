...
import Wsocket, { debounce } from './socket'
...
export default class camera {
  ...
  eventAgent () {
    ...
    document.body.addEventListener('mouseup', ev => {
      // this.observer({type:'click', evt: ev})
      if (
        this.mousedownPoint.clientX === ev.clientX &&
        this.mousedownPoint.clientY === ev.clientY
      ) {
        this.observer({ type: 'click', evt: ev })
      } else {
  ...
  lazy () {
    ...
    xhr.open(
      'GET',
      `https://static.zhongan.com/upload/mobile/material/1480043510064.png'/?v=${Math.random()}`,
      false
    )
    xhr.send(null)
  }
